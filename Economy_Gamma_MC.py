...
class Economy_Gamma_MC(Economy):
    ...
    def __init__(self, misClassificationCost, timeCost, min_t, classifier, nbIntervals, order, sampling_ratio, use_complete_ECO_model, pathECOmodel, fears, dataset , feat, folderRealData, aggregate):
        super().__init__(misClassificationCost, timeCost, min_t, classifier)
        self.method = "Gamma_MC"
    ...
    def aggregateProbaVector(self, proba_vector):
        """
        Aggregates a multiclass stochastic output vector into one value using a function
        """
        if self.aggregate == 'margins':
            return sigmoid(margins(proba_vector))
        elif self.aggregate == 'gini':
           return sigmoid(giniImpurity(proba_vector))
        elif self.aggregate == 'entropy':
            return sigmoid(entropyFunc1(proba_vector))
        elif self.aggregate.startswith("renyi"):
            return sigmoid(entropyRenyi(proba_vector, float(self.aggregate.split('_')[1])))
        elif self.aggregate == "variance":
            return variance(proba_vector)
        elif self.aggregate.startswith("custom"):
            return sigmoid(custom1(proba_vector, float(self.aggregate.split('_')[1])))
        elif self.aggregate == "max":
            return sigmoid(max(proba_vector))
    ...
    def forecastExpectedCost(self, x_t, pb):
        ...
        for i,t in enumerate(self.timestamps[self.timestamps.index(t_current):]):
            ...
            # print("DEBUG", self.dataset, self.nbIntervals, "Gamma_MC score loop", "index", i,  "time", t)
    ...
    def findInterval(self, x_t, pb=None):
        ...
        proba  = self.aggregateProbaVector(proba)
